...
public interface StudyFolderRepository extends JpaRepository<StudyFolder, Long> {
        ...
        // Find all root folders - no need for EntityGraph since parentFolder is null
        @Query("SELECT f FROM StudyFolder f WHERE f.userId = :userId AND f.parentFolder IS NULL")
        List<StudyFolder> findRootFoldersWithAssociations(@Param("userId") Long userId);
        ...
        // Count subfolders for a parent folder
        long countByParentFolderId(Long parentFolderId);

        // Get parent folder ID without triggering lazy loading
        @Query("SELECT f.parentFolder.id FROM StudyFolder f WHERE f.id = :folderId")
        Long getParentFolderId(@Param("folderId") Long folderId);
}
